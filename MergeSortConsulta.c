...
static void mergeSortRecursivoConsulta(
    FILE *arq,
    int inicio,
    int fim
) {
    ...
    mergeSortRecursivoConsulta(arq, inicio, meio);
    mergeSortRecursivoConsulta(arq, meio + 1, fim);

    Tconsulta regEsq;
    Tconsulta regDir;
    Tconsulta regTemp;

    size_t tam = tamanho_registro_consulta();
...
void mergeSortDiscoConsulta(
    ...
    mergeSortRecursivoConsulta(arq, 0, totalRegistros - 1);
    ...
    if (log != NULL) {
        ...
        fprintf(log, "Tempo de execucaço: %.6f segundos\n", tempo);
        fprintf(log, "------------------------------\n");
    }
}
